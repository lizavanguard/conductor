...
namespace Conductor.Game.Model
{
    ...
    public class CaptainAI
    {
        ...
        // Nodeの自動生成に着手 kokokara
            ...
            // だるくなったらtsvとか外部ツールに逃がせばいいような気もする
        ...
        // 試しにtargetPositionガン守りと攻撃の部隊を作ってみる
            ...
            // Nodeの生成部分、ぶっちゃけ普通に全パターン書くとだるすぎるので、外部データから自動生成したい
                // あるOperationに関して、1. Conditionをfalse->trueと変化する、2. 無関係、3. 終わった後にtrueとは限らない(処理上は「必ずfalseになる」という扱いで進める)の三種類に分類
                // 1はfalse->true、3はfalse->falseとした上で、2に関して全網羅
                // 例えばConditionがA～Dの四種類で、あるOperationに関してAは1、BCは2、Dが3に分類される場合、四個のNodeができあがる
                // 攻撃している、というような決して達成されない系は、一部のOperationで1を取り、それ以外は全て3をとる こうすることで、このNodeは必ず最後にしか置かれなくなる
                // goalとして達成不可Conditionを二つ以上持つようなものはNGになるかも
